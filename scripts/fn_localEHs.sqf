	...
	if (typeOf _object isEqualTo FOBOBJECT) then {
		...
		// Create fob markers + sound
		{
			if (side _x isEqualTo side _unit) then {
				...
				_fobMarker setMarkerTextLocal format ["FOB %1",_fobName];
		...
		_onAct = format ["if (side player isEqualTo %2) then { hint '%1 is under attack'};",_object getVariable "fobName",_hintSide];
